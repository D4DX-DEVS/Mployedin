...
// Reached through getLiveAgentPerformance, which counts the six agent
// scorecard figures from their own collections instead of Agent.performance.
jest.mock("@/models/Interview", () => ({ __esModule: true, default: { countDocuments: jest.fn().mockResolvedValue(0), find: jest.fn().mockReturnValue({ select: jest.fn().mockReturnValue({ lean: jest.fn().mockResolvedValue([]) }), lean: jest.fn().mockResolvedValue([]) }), aggregate: jest.fn().mockResolvedValue([]) } }));
