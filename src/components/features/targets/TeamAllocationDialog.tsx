...
export function TeamAllocationDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] max-w-[900px] flex-col gap-0 overflow-hidden p-0 sm:max-w-[900px]">
        {/* Header */}
          ...
          <div className="space-y-5">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant={c.remain < 0 ? "destructive" : "success"}
                        className="text-[10px]"
                      >
                        {c.remain < 0 ? "Over" : "OK"}
                      </Badge>
                    ...
                    <div className="mt-1 flex items-center justify-between text-[11px] text-muted-foreground">
                      <span>Allocated: {c.fmt(c.allocated)}</span>
                      <span
                        className={
                          c.remain < 0
                    ...
                    <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted/60">
                      <div
                        className={`h-full rounded-full transition-all ${c.remain < 0 ? "bg-red-500" : "bg-sky-500"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
            ...
            <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
              ...
              <div className="rounded-xl border border-border/60 bg-card p-3">
                ...
                <div className="mt-2 max-h-[340px] space-y-1.5 overflow-y-auto">
                  {visibleAgents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`font-semibold tabular-nums ${pct >= 75 ? "text-emerald-600" : pct >= 40 ? "text-amber-600" : "text-red-500"}`}
                        >
                          {pct}%
                        </span>
                      </label>
        ...
        {/* Footer */}
        <DialogFooter className="flex-shrink-0 border-t border-border/60 bg-background px-6 py-3 sm:justify-between">
          <div className="flex min-h-8 flex-1 items-center text-xs text-muted-foreground">
            {errors.length > 0 ? (
              <span className="text-red-500">{errors[0]}</span>
            ) : (
              <span>
                {rows.length} agents · {mode} distribution
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {saving
                ? "Distributing…"
                : `Distribute to ${rows.length} Agents`}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
