  ...
  return (
    <section className="workspace-panel-surface overflow-hidden rounded-2xl sm:rounded-3xl">
      <div className="border-b border-border/60 px-3 sm:px-5 py-5 sm:px-3 sm:px-6">
        ...
        <h2 className="heading-section mt-3 font-semibold tracking-tight text-foreground">{t("jumpToQueues")}</h2>
      ...
      <div className="space-y-2.5 px-3 py-3 sm:px-5 sm:py-5">
        <Link
          ...
          className="group flex items-center gap-3 rounded-3xl border border-border bg-background/80 transition-all hover:-translate-y-0.5 hover:border-sky-500/25 card-pad"
        >
...
export function OpeningsStats({ activeJobs, totalApplications }: OpeningsStatsProps) {
  ...
  return (
    <section className="workspace-panel-surface rounded-2xl sm:rounded-3xl panel-body">
      ...
      <h2 className="heading-section mt-2 font-semibold tracking-tight text-foreground">{t("seeHiringVolume")}</h2>
      <div className="mt-4 space-y-2.5">
        <div className="rounded-2xl border border-border bg-background/80 card-pad">
...
export function TimeToHire({ avgDays }: TimeToHireProps) {
  ...
  return (
    <section className="rounded-3xl border border-amber-200 bg-[linear-gradient(180deg,_rgba(255,251,235,0.98),_rgba(255,247,237,0.96))] shadow-[0_24px_60px_-46px_rgba(245,158,11,0.32)] panel-body">
      <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-amber-700">
        <Clock className="h-3.5 w-3.5" />
        {t("timeToHire")}
      </div>
      <h2 className="heading-section mt-2 font-semibold tracking-tight text-foreground">{t("monitorSpeed")}</h2>
      {avgDays !== null ? (
        <>
