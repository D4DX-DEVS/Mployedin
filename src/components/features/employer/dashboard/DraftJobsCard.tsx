...
export function DraftJobsCard({ locale, variant = "card", onCountChange }: DraftJobsCardProps) {
  ...
  if (variant === "banner") {
    return (
      ...
      <div className="workspace-glass-panel card-pad flex h-full flex-wrap items-center gap-x-3 gap-y-2 rounded-2xl border border-amber-200/60">
        {ConfirmDialogNode}
        ...
        <div className="-mx-1 flex min-w-0 basis-full gap-2 overflow-x-auto px-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:ms-auto sm:basis-auto sm:flex-wrap sm:overflow-visible">
          {drafts.slice(0, 3).map((d) => (
            <Button
              ...
              size="sm"
              variant="outline"
              className="h-7 shrink-0 gap-1 rounded-lg border-amber-200 bg-background/70 px-2 text-[11px] text-amber-700 sm:h-8 sm:gap-1.5 sm:px-3 sm:text-xs hover:bg-amber-50"
    ...
    <section className="workspace-panel-surface panel-body flex flex-col rounded-3xl">
      ...
      <ul className="space-y-2">
        {drafts.slice(0, 3).map((d) => (
          <li
            key={d._id}
            className="group flex items-center gap-2 rounded-2xl border border-border/60 bg-background/70 transition-all hover:-translate-y-px hover:border-amber-400/40 hover:shadow-sm chip-pad"
          >
            <Link href={continueHref(d._id)} className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <span className="hidden shrink-0 items-center gap-1.5 rounded-lg bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700 transition-colors group-hover:bg-amber-100 sm:inline-flex">
                <RotateCcw className="h-3.5 w-3.5" />
                {t("continueEditing")}
              </span>
            </Link>
