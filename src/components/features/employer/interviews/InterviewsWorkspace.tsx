...
interface InterviewsWorkspaceProps {
  jobId?: string;
  embedded?: boolean;
}

export function InterviewsWorkspace({ jobId: propJobId, embedded = false }: InterviewsWorkspaceProps) {
  ...
  const [urlJobFilter, setUrlJobFilter] = useUrlFilter("jobId", "");
  const jobFilter = propJobId ?? urlJobFilter;
  ...
  async function cancelInterview(iv: Interview) {
    ...
    if (ok) updateMutation.mutate({ id: iv._id, status: "cancelled" });
  }
  ...
  function getOutcomeLabel(outcome?: string) {
    switch (outcome) {
      case "passed": return { label: t("passed"), color: "text-emerald-700 bg-status-selected-bg border-status-selected/20" };
      case "failed": return { label: t("rejected"), color: "text-status-rejected bg-status-rejected-bg border-status-rejected/20" };
      case "hold": return { label: t("onHold"), color: "text-status-shortlisted bg-status-shortlisted-bg border-status-shortlisted/20" };
      case "no_show": return { label: t("noShow"), color: "text-muted-foreground bg-secondary/75 border-border" };
      default: return null;
    }
  }
  ...
  return (
    <div className={embedded ? "space-y-3 sm:space-y-4" : "page-container"}>
      {ConfirmDialogNode}
      {aiTarget && (
      ...
      {modal.kind !== "none" && (
        <InterviewActionModal
          ...
          locale={locale}
          onOfferCreated={() => {
            if (embedded && propJobId) {
              router.push(`/${locale}/employer/jobs/${propJobId}/offers`);
            } else {
              router.push(`/${locale}/employer/offers`);
            }
          }}
        />
      )}
      ...
      {!embedded && (
        <>
          ...
          />
          {jobFilter && <JobScopeStrip jobId={jobFilter} locale={locale} onClear={() => setUrlJobFilter("")} />}
        </>
      )}

      {/* ── Embedded mode: compact toolbar with status chips and actions ──── */}
      {embedded && (
        <div className="workspace-toolbar flex-wrap">
          ...
          >
            ...
            <span className="tabular-nums">{formatNumber(scheduledTotal, locale)}</span>
          </button>
          <button
            type="button"
            onClick={() => { setStatus(status === "completed" ? "" : "completed"); setPage(1); }}
          ...
          >
            ...
            <span className="tabular-nums">{formatNumber(completedTotal, locale)}</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span className="sm:hidden">{t("attentionShort")}</span>
            <span className="hidden sm:inline">{t("needsAttention")}</span>
            <span className="tabular-nums">{formatNumber(attentionTotal, locale)}</span>
          </button>
          <button
            type="button"
            onClick={() => { setStatus(status === "confirmed" ? "" : "confirmed"); setPage(1); }}
          ...
          >
            ...
            <span className="tabular-nums">{formatNumber(confirmedTotal, locale)}</span>
          </button>

          {/* Actions: Bulk schedule (primary), Calendar, Scorecards, Export — collapse secondary on phones */}
          <div className="ms-auto flex flex-wrap items-center gap-2">
            {can("interviews", "create") ? (
              <Button
                asChild
                className="gap-2 rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground hover:bg-primary/90 sm:px-4 sm:text-sm min-h-11 sm:min-h-9"
              >
                <Link href={`/${locale}/employer/interviews/bulk?jobId=${propJobId}`} aria-label={t("bulkSchedule")}>
                  <span className="hidden sm:inline">{t("bulkSchedule")}</span>
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
            ...
            {/* Secondary actions on desktop, menu on mobile */}
            <div className="hidden sm:flex gap-2">
              <Button
                variant="outline"
                asChild
                className="rounded-xl px-3 text-xs font-semibold sm:px-4 sm:text-sm gap-2 min-h-9"
              >
                <Link href={`/${locale}/employer/calendar`} aria-label={t("viewCalendar")}>
                  <CalendarDays className="h-4 w-4" />
                  <span className="hidden lg:inline">{t("viewCalendar")}</span>
                </Link>
              </Button>
              <Button
                variant="outline"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" aria-label={tn("more")} className="rounded-xl px-3 sm:hidden min-h-11">
                  <MoreHorizontal className="h-4 w-4" aria-hidden="true" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuItem asChild>
                  <Link href={`/${locale}/employer/calendar`}>
                    <CalendarDays className="me-2 h-4 w-4" />
                    {t("viewCalendar")}
                  </Link>
                </DropdownMenuItem>
                  ...
                  <TableRow key={iv._id} className="hover:bg-secondary/50">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs capitalize text-muted-foreground">{iv.type ?? "in-person"}</span>
                      </div>
                      {(() => {
