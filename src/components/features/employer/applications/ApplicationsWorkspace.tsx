...
export function ApplicationsWorkspace({
  ...
  const initialJobId = jobId ?? searchParams.get("jobId") ?? searchParams.get("job") ?? "";
  ...
  // Every URL write goes through here. router.replace commits asynchronously,
  // so two writers in one tick — a preset applying its filters, then the
  // filter-reset effect clearing the page — both read window.location, and
  // the second one silently dropped the first one's query ("Top matches"
  // never reached the URL). The last written params stay the base until the
  // router has caught up.
  const pendingUrlRef = useRef<URLSearchParams | null>(null);
  const searchKey = searchParams.toString();
  useEffect(() => { pendingUrlRef.current = null; }, [searchKey]);
  function writeUrl(mutate: (params: URLSearchParams) => void) {
    const params = new URLSearchParams(pendingUrlRef.current ?? window.location.search);
    mutate(params);
    pendingUrlRef.current = params;
  ...
  function setPage(next: number) {
    setPageState(next);
    writeUrl((params) => { if (next > 1) params.set("page", String(next)); else params.delete("page"); });
  }
  const [limit, setLimit] = useState(10);
  // ?status=applied deep-links from the dashboard "New Applications" card
  const [statusFilter, setStatusFilter] = useState(() => searchParams.get("status") ?? "all");
  // ?unreviewed=1 deep-links from the Overview "not yet reviewed" row (A17).
  const [unreviewedOnly, setUnreviewedOnly] = useState(() => ["1", "true"].includes(searchParams.get("unreviewed") ?? ""));
  ...
  // Seeded from the URL like score/search/status, so a saved view's link
  // (?experienceMin=3&skills=React&sort=score) restores the whole view.
  const [experienceRange, setExperienceRange] = useState<[number | null, number | null]>(() => {
    const emin = searchParams.get("experienceMin");
    const emax = searchParams.get("experienceMax");
  ...
  const view: "list" | "board" = Boolean(jobFilter) && searchParams.get("view") === "board" ? "board" : "list";
  function hrefWithView(next: "list" | "board") {
  ...
  // Talent pools key on the seeker, not the application; de-duplicate across rows.
  const selectedSeekerIds = Array.from(new Set(
    selected.map((id) => applications.find((a) => a._id === id)?.jobSeekerId?._id).filter((x): x is string => Boolean(x)),
  ));
    ...
    <div className={embedded ? "space-y-3 sm:space-y-4" : "page-container"}>
      ...
      <div className={embedded ? "workspace-toolbar is-embedded" : "workspace-toolbar"}>
        ...
        />
          ...
          {!embedded && selectedJob && (
            <Link href={`/${locale}/employer/jobs/${selectedJob._id}/applications`} className="text-xs text-muted-foreground hover:text-foreground">
              {tw("openThisJob")}
            </Link>
          )}
      ...
      {canUpdate && selected.length > 0 && (
        <div className="flex flex-col gap-3 rounded-3xl border border-sky-500/20 bg-sky-500/10 text-sky-800 card-pad sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="outline" className="h-10 flex-1 rounded-xl border-border bg-background/80 px-2 text-xs sm:flex-none sm:px-4 sm:text-sm"
              onClick={() => setBulkPoolOpen(true)} disabled={selectedSeekerIds.length === 0}>
              <FolderPlus className="h-3.5 w-3.5 sm:me-2" aria-hidden="true" />
      ...
      <SaveToPoolDialog
        candidateId={null}
        candidateIds={selectedSeekerIds}
        open={bulkPoolOpen}
      ...
      >
        <div className="min-w-0 space-y-3">
          ...
          {(canUpdate || Boolean(jobFilter)) && (
            <div className="flex flex-wrap items-center gap-2 [&>button]:min-w-0">
              {/* List ⇄ Board lives here, not in the toolbar, so the toolbar stays one row on laptops. */}
              {jobFilter ? (
                <div>
                  <ViewToggle
                    active={view}
                    ariaLabel={tw("viewToggleLabel")}
                    options={[
                      { key: "list", href: hrefWithView("list"), label: tw("viewList"), icon: LayoutList },
                      { key: "board", href: hrefWithView("board"), label: tw("viewBoard"), icon: Columns3 },
                    ]}
                  />
                </div>
              ) : null}
      ...
      {view === "list" && !isLoading && !applicationsQuery.isError && (
        <PaginationControls
          page={page}
