...
export function OffersWorkspace({ jobId, embedded = false }: OffersWorkspaceProps) {
  ...
  // Fetch employer jobs for the filter dropdown (skip when embedded)
  useEffect(() => {
    if (embedded) return;

    fetch("/api/jobs?limit=100&fields=title&myJobs=true")
  ...
  const containerClass = embedded ? "space-y-3 sm:space-y-4" : "page-container";
