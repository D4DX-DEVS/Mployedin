...
import {
  Sparkles, Clock3, CheckCircle2, ReceiptText,
...
export default function SuperAgentInvoicesPage() {
  ...
  return (
    <div className="page-container space-y-6">
      <TableToolbar
        title="Invoices"
        description="Track team recruitment invoices and payment status across your agents."
        search=""
        onSearchChange={() => {}}
        ...
        filterContent={(
          <div className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div>
                <label htmlFor="sa-inv-status" className="sr-only">Filter by status</label>
                <SearchableSelect id="sa-inv-status" className="h-11 w-full rounded-xl border-border bg-card" options={STATUS_OPTIONS} value={status || "all"} onValueChange={(v) => { setStatus(v === "all" ? "" : v); resetPage(); }} placeholder="All statuses" />
              </div>
              <div>
                <label htmlFor="sa-inv-cat" className="sr-only">Filter by category</label>
                <SearchableSelect id="sa-inv-cat" className="h-11 w-full rounded-xl border-border bg-card" options={CATEGORY_OPTIONS} value={category || "all"} onValueChange={(v) => { setCategory(v === "all" ? "" : v); resetPage(); }} placeholder="All categories" />
              </div>
      ...
      {errorMessage ? (
        <div className="rounded-2xl border border-rose-200 bg-rose-50/90 px-4 py-3 text-sm text-rose-700 shadow-sm dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-200">{errorMessage}</div>
      ) : null}
                  ...
                  <TableCell colSpan={6} className="px-6 py-14 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">Team invoices will appear here when agents create them.</p>
                      </div>
                    </div>
                  </TableCell>
                </TableRow>
