  ...
  return (
    <div className="page-container agent-legacy-surface space-y-6">
      <section className="workspace-hero-surface overflow-hidden rounded-[28px] p-6 sm:p-7">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Structured performance metrics, lead funnel analysis, and AI-powered report generation — all from your live data.</p>
          </div>
          <div className="workspace-glass-panel rounded-2xl px-4 py-3 text-left sm:min-w-[260px]">
            ...
            <p className="mt-1 text-lg font-semibold text-foreground">{analytics?.kpis.totalLeads ?? "—"} leads total</p>
            <p className="text-xs text-muted-foreground">Live structured data with 30-day trend comparisons.</p>
          </div>
        </div>
      </section>
      ...
      {analyticsLoading ? (
        ...
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              { label: "Leads (30d)", value: analytics.trends.leads.current, trend: analytics.trends.leads, icon: <Target className="h-5 w-5" />, tone: "workspace-tone-sky" },
              { label: "Applications (30d)", value: analytics.trends.applications.current, trend: analytics.trends.applications, icon: <Users className="h-5 w-5" />, tone: "workspace-tone-indigo" },
              { label: "Placements (30d)", value: analytics.trends.placements.current, trend: analytics.trends.placements, icon: <TrendingUp className="h-5 w-5" />, tone: "workspace-tone-emerald" },
              { label: "Interviews (30d)", value: analytics.trends.interviews.current, trend: analytics.trends.interviews, icon: <CalendarCheck2 className="h-5 w-5" />, tone: "workspace-tone-amber" },
            ].map((kpi) => (
          ...
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              { label: "Active Jobs", value: analytics.kpis.activeJobs, icon: <Briefcase className="h-4 w-4" /> },
              { label: "Employers", value: analytics.kpis.employers, icon: <Building2 className="h-4 w-4" /> },
              { label: "Overdue Follow-ups", value: analytics.kpis.overdueFollowUps, icon: <Flame className="h-4 w-4" />, alert: analytics.kpis.overdueFollowUps > 0 },
              { label: "Scheduled Interviews", value: analytics.kpis.scheduledInterviews, icon: <CalendarCheck2 className="h-4 w-4" /> },
            ].map((s) => (
          ...
          <section className="workspace-panel-surface rounded-[28px] p-5">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Lead funnel</p>
            <h2 className="mt-2 text-xl font-semibold tracking-tight text-foreground">Pipeline Stage Distribution</h2>
              ...
              {FUNNEL_STAGES.map((stage) => {
                ...
                return (
                  <div key={stage} className="flex items-center gap-3">
                    <span className="w-24 text-xs font-medium capitalize text-muted-foreground">{stage}</span>
          ...
          <section className="workspace-panel-surface rounded-[28px] p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Commission Summary</p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { label: "Pending", data: analytics.commissions.pending, color: "text-amber-600" },
                { label: "Approved", data: analytics.commissions.approved, color: "text-sky-600" },
                { label: "Paid", data: analytics.commissions.paid, color: "text-emerald-600" },
              ].map((c) => (
          ...
          {analytics.monthlyTrends.length > 0 && (
            <section className="workspace-panel-surface rounded-[28px] p-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Monthly Trends</p>
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-foreground">6-Month Performance</h2>
      ...
      <section className="workspace-panel-surface rounded-[28px] p-5 sm:p-6">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">AI-powered reports</p>
        <h2 className="mt-2 text-xl font-semibold tracking-tight text-foreground">Generate custom reports from your live data</h2>
        <p className="mt-1 text-sm text-muted-foreground">Reports are generated using your actual database figures — no estimated or external data.</p>
