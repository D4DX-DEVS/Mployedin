...
const IMPORT_TYPES = [
  ...
  { value: "employers", label: "Employers", icon: <Building2 className="h-4 w-4" /> },
];
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult(data);
        setStep(3);
        toast.success(`Import complete: ${data.success} records imported`);
      } else {
      ...
      <section className="workspace-hero-surface overflow-hidden rounded-[28px] p-6 sm:p-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Import users, jobs, or employers via CSV file</p>
          </div>
        </div>
      ...
      {step === 1 && (
        <section className="workspace-panel-surface rounded-[28px] p-6">
          <div className="space-y-6">
            {/* Type selection */}
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">Import Type</label>
              <div className="flex gap-3">
                {IMPORT_TYPES.map((t) => (
                  <button
                    key={t.value}
                    onClick={() => setImportType(t.value as ImportType)}
            ...
            <div className="workspace-glass-panel rounded-2xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Get the CSV template with correct headers for {importType}</p>
                </div>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Expected columns: <code className="rounded bg-muted px-1 py-0.5">{TEMPLATES[importType].join(", ")}</code>
              </p>
            </div>
            ...
            >
              ...
              <p className="mt-4 text-sm font-medium text-foreground">Click to upload CSV file</p>
              <p className="mt-1 text-xs text-muted-foreground">Maximum 5MB · CSV format only</p>
              <input
                ref={fileRef}
                type="file"
                accept=".csv"
        ...
        <section className="workspace-panel-surface rounded-[28px] p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{file?.name} · {parsedRows.length} rows found</p>
            </div>
            <div className="flex items-center gap-3">
      ...
      {step === 3 && result && (
        <section className="workspace-panel-surface rounded-[28px] p-6">
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <h2 className="mt-4 text-xl font-semibold text-foreground">Import Complete</h2>
            ...
            {result.errors.length > 0 && (
              <div className="mt-4 w-full max-w-md text-left">
                <p className="mb-2 text-sm font-medium text-foreground">Errors:</p>
            ...
            <Button className="mt-6" onClick={reset}>
              <RotateCcw className="mr-1 h-3.5 w-3.5" /> Import More
            </Button>
          </div>
        </section>
      )}
