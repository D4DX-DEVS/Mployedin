...
test.describe("Job Seeker Support Ticket", () => {
  ...
  test("support page renders correctly", async ({ page }) => {
    ...
    await expect(page.getByText("Support")).toBeVisible({ timeout: 10_000 });
  ...
  test("can create a support ticket and send a message", async ({ page }) => {
    ...
    // Send a follow-up message in the ticket conversation
    ...
    if (await messageInput.isVisible()) {
      ...
      await expect(page.getByText(followUpMessage)).toBeVisible({ timeout: 10_000 });
    }
  });
